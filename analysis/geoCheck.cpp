...
int main() {
    ...
    for (int noEta = 0; noEta < 92; noEta++) {
        ...
        if (noEta < 52) pParamBase = static_cast<dd4hep::DDSegmentation::DRparamBase*>(pParamBarrel);
        else pParamBase = static_cast<dd4hep::DDSegmentation::DRparamBase*>(pParamEndcap);
        pParamBase->SetDeltaThetaByTowerNo(noEta, pParamBarrel->GetTotTowerNum());
        pParamBase->SetThetaOfCenterByTowerNo(noEta, pParamBarrel->GetTotTowerNum());
        pParamBase->SetIsRHSByTowerNo(noEta);
        pParamBase->SetCurrentTowerNum(noEta);
        pParamBase->init();
        ...
        std::string file_name = "tower_geo_" + std::to_string(noEta) + ".txt";
        out_file.open(file_name);
        out_file << beamTheta << std::endl;
        out_file << beamPhi   << std::endl;
        out_file << ( (beamStartingPoint_x - x_beamSpotSize) * 10. ) << std::endl;
        out_file << ( (beamStartingPoint_x + x_beamSpotSize) * 10. ) << std::endl;
        out_file << ( (beamStartingPoint_y - y_beamSpotSize) * 10. ) << std::endl;
        out_file << ( (beamStartingPoint_y + y_beamSpotSize) * 10. ) << std::endl;
        out_file << ( (beamStartingPoint_z - z_beamSpotSize) * 10. ) << std::endl;
        out_file << ( (beamStartingPoint_z + z_beamSpotSize) * 10. ) << std::endl;
        out_file.close();
    }
}